...
#define CHECK_VALUE(Value, Message) if(FMath::IsFinite(VertexValues[Value])==false) UE_LOG(PointCloudLog, Warning, TEXT("Found Nan or Infinite on Vertex %d Value %s"), Index, *Message);
...
void UPointCloudImpl::InitDb()
{
	...
	if (IsInitialized())
	{
		UE_LOG(PointCloudLog, Warning, TEXT("Init DB Called On Initialized PointCloud: %s\n"));
		return;
	}
	...
	if (rc)
	{
		UE_LOG(PointCloudLog, Warning, TEXT("Can't open database: %s\n"), sqlite3_errmsg(InternalDatabase));
		sqlite3_close(InternalDatabase);
		InternalDatabase = 0;
		return;
	}
...
void UPointCloudImpl::SerializeDb(FArchive& Ar)
{
	...
	Ar << Size;
	Ar.SerializeCompressed(Data, Size, NAME_Zlib);
...
void UPointCloudImpl::DeSerializeDb(FArchive& Ar)
{
	...
	Ar.SerializeCompressed(Copy, Size, NAME_Zlib);
	Ar.Serialize(WholeDbHash.m_digest, WholeDbHash.DigestSize);
