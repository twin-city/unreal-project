...
FPointCloudCsv::FPointCloudCsv(const TArray<FString> &InStrings, FFeedbackContext* Warn)
{
	...
	if (InStrings.Num() < 2)
	{
		UE_LOG(PointCloudLog, Warning, TEXT("Malformed CSV. Less than 2 Lines %s\n"));				
		IsOpen = false;
		return;
	}
	...
	if (InStrings[0].ParseIntoArray(ColumnNames, TEXT(","), true) == 0)
	{
		...
		UE_LOG(PointCloudLog, Warning, TEXT("Malformed CSV. Cannot Read Column Names From Line 0 %s\n"));				
		IsOpen = false;
		return;
	}
