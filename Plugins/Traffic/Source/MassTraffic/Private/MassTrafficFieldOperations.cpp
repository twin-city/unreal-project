...
{
	if (UMassTrafficSubsystem* TrafficSubsystem = Context.GetMutableSubsystem<UMassTrafficSubsystem>(EntityManager.GetWorld()))
	{
...
void UMassTrafficFrameStartFieldOperationsProcessor::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& Context)
{
	// Re-cache lanes & intersections for Movable traffic fields
	{
		...
		UMassTrafficSubsystem& TrafficSubsystem = Context.GetMutableSubsystemChecked<UMassTrafficSubsystem>(EntityManager.GetWorld());
