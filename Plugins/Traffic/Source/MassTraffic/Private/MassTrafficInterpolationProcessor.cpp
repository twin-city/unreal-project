...
void UMassTrafficInterpolationProcessor::Execute(FMassEntityManager& EntityManager, FMassExecutionContext& Context)
{
	...
	EntityQueryNonOffLOD_Conditional.ForEachEntityChunk(EntityManager, Context, [&, World = EntityManager.GetWorld()](FMassExecutionContext& QueryContext)
	{
		...
		for (int32 Index = 0; Index < NumEntities; ++Index)
		{
			...
			// Debug
			UE::MassTraffic::DrawDebugInterpolatedAxles(GetWorld(), FrontAxleTransform.GetLocation(), RearAxleTransform.GetLocation(), bVisLog, LogOwner);
			...
			// Debug
			UE::MassTraffic::DrawDebugLaneSegment(GetWorld(), VehicleMovementInterpolationFragment.LaneLocationLaneSegment, bVisLog, LogOwner);
		}
	});
